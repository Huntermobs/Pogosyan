...
class Vkads_stats_api:
    ...
    def __init__(self, general_creds='creds_vkads.txt',
                 tokens_folder=os.getcwd()+'\\token\\', acc_ids=['']):
        ...
        for acc_id in acc_ids:
            for _, _, files in os.walk(tokens_folder):
                if acc_id + '.txt' in files:
                    with open(tokens_folder + acc_id + '.txt', "r", encoding='utf8') as f:
                        token = json.load(f)
                        token = token["access_token"]
                else:
                    with open(tokens_folder + general_creds, "r", encoding='utf8') as f:
                        credentials = json.load(f)
                    token_response = self._reguest_token(credentials=credentials, acc_id=acc_id)
                    with open(tokens_folder + acc_id + '.txt', "w", encoding="utf-8") as output_file:
        ...
        for root, dirs, files in os.walk(tokens_folder):
            if 'account_token' + '.txt' in files:
                with open(tokens_folder + 'account_token' + '.txt', "r", encoding='utf8') as f:
                    token = json.load(f)
                ...
                with open(tokens_folder + 'account_token' + '.txt', "w", encoding="utf-8") as output_file:
                    json.dump(token_response, output_file)
    ...
    def _reguest_token(self, credentials, acc_id):
        ...
        if response.status_code == 200:
            ...
            print(response.json())
            raise ValueError ('Вызвана ошибка запроса: ' + response.json())

    def _refresh_token(self, acc_id):

        with open(self.tokens_folder + self.general_creds, "r", encoding='utf8') as f:
            credentials = json.load(f)
        if acc_id:
            with open(self.tokens_folder + acc_id + '.txt', "r", encoding='utf8') as f:
                refresh_token = json.load(f)
                refresh_token = refresh_token["refresh_token"]
        else:
            with open(self.tokens_folder + 'account_token' + '.txt', "r", encoding='utf8') as f:
                refresh_token = json.load(f)
                refresh_token = refresh_token["refresh_token"]
        url = self.url + "oauth2/token.json"
        payload = {
        ...
        if response.status_code == 200:
            if acc_id:
                with open(self.tokens_folder + acc_id + '.txt', "w", encoding="utf-8") as output_file:
                    json.dump(response.json(), output_file)

                    self.tokens[acc_id] = response.json()["access_token"]
            else:
                with open(self.tokens_folder + 'account_token' + '.txt', "w", encoding="utf-8") as output_file:
                    json.dump(response.json(), output_file)
